...
export function DownloadProvider({ children }: PropsWithChildren) {
  ...
  const run = useCallback(
    async (id: string, video: VideoSummary) => {
      try {
        ...
        if (result) {
          ...
          try {
            const thumbnail = await File.downloadFileAsync(
              video.thumbnail,
              new File(downloadsDirectory, `${video.bvid}.jpg`),
              { idempotent: true },
            );
            thumbnailUri = thumbnail.uri;
          } catch {
            /* The video remains usable if artwork cannot be cached. */
          }
          update(id, {
            status: 'completed',
            progress: 1,
      ...
      try {
        const resumable =
          task?.state === 'paused'
            ? task
            : item.pauseState
              ? DownloadTask.fromSavable(item.pauseState, {
                  onProgress: ({ bytesWritten, totalBytes }) =>
                    update(id, {
                      bytes: bytesWritten,
                      totalBytes,
  ...
  const value = useMemo(
    () => ({ downloads, start, pause, resume, remove }),
    [downloads, start, pause, resume, remove],
  );
