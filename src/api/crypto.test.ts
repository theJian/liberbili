...
describe('Bilibili crypto compatibility', () => {
  ...
  test('signing is deterministic for a fixed clock', () => {
    const signed = signWbi({ bvid: 'BV1xx411c7mD', cid: 123 }, '12345678901234567890123456789012', 1_700_000_000_000);
    expect(signed).toContain('wts=1700000000');
    expect(signed).toMatch(/w_rid=[a-f0-9]{32}$/);
    expect(signWbi({ cid: 123, bvid: 'BV1xx411c7mD' }, '12345678901234567890123456789012', 1_700_000_000_000)).toBe(signed);
  });
