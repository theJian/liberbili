...
function mapError(code: number, message = 'Bilibili request failed') {
  ...
  if (message.includes('地区'))
    return new BilibiliError('restricted', message, code);
...
export const recommendationsUrl = () =>
  `${API}/x/web-interface/index/top/rcmd?fresh_type=3`;

export function normalizeSummary(item: Json): VideoSummary {
  const bvid =
    item.bvid ||
    item.uri?.match(/BV[\w]+/)?.[0] ||
    item.arcurl?.match(/BV[\w]+/)?.[0];
  return {
    bvid,
    ...
    duration:
      typeof item.duration === 'string'
        ? item.duration
            .split(':')
            .reduce((sum: number, value: string) => sum * 60 + Number(value), 0)
        : Number(item.duration ?? 0),
    views: Number(item.stat?.view ?? item.play ?? 0),
    publishedAt: Number(item.pubdate ?? 0) || undefined,
    requiresMembership:
      item.elec_arc_type === 1 ||
      JSON.stringify(item.badges ?? []).includes('充电专属'),
    type: 'video',
  };
}
...
function normalizeComment(item: Json): Comment {
  ...
  const jumpUrl = Object.keys(item.content?.jump_url ?? {}).find((url) =>
    url.startsWith('https://'),
  );
  if (message.endsWith('...') && jumpUrl) message += `\n\n${jumpUrl}`;
  const pictureDetails = (item.content?.pictures ?? []).map(
    (picture: Json) => ({
...
export class BilibiliClient {
    ...
    try {
      ...
      if (!response.ok)
        throw new BilibiliError(
          response.status === 412 ? 'antiBot' : 'network',
  ...
  async getRecommendations() {
    ...
    return (data.item ?? [])
      .filter((item: Json) => item.bvid || item.uri?.includes('/video/'))
  ...
  async getVideo(bvid: string): Promise<VideoDetail> {
    ...
    return {
      ...
      stats: Object.fromEntries(
        Object.entries(data.stat ?? {}).map(([key, value]) => [
          key,
  ...
  async getComments(aid: number, cursor = ''): Promise<PageResult<Comment>> {
    ...
    const data = await this.request<Json>(url);
    const items = [
      ...(data.top_replies ?? []).map((item: Json) => ({
        ...item,
        isTop: true,
      })),
      ...(data.replies ?? []),
    ].map(normalizeComment);
    const next = data.cursor?.is_end
      ? undefined
