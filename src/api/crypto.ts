...
export function bvToAv(bvid: string) {
  ...
  const decoded = value
    .slice(3)
    .reduce(
      (total, character) => total * 58n + BigInt(BV_TABLE.indexOf(character)),
      0n,
    );
...
export function mixinKey(imgKey: string, subKey: string) {
  ...
  return MIXIN_INDEX.map((index) => source[index])
    .join('')
    .slice(0, 32);
}
...
export function signWbi(
  ...
  const entries = Object.entries({ ...params, wts }).sort(([a], [b]) =>
    a.localeCompare(b),
  );
  const query = entries
...
export function ticketSignature(timestamp: number) {
  return bytesToHex(
    hmac(sha256, utf8ToBytes('XgwSnGZ1p'), utf8ToBytes(`ts${timestamp}`)),
  );
}
