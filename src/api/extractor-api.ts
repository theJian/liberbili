...
const AUDIO_QUALITY: Record<number, string> = {
  30216: '64K',
  30232: '132K',
  30280: '192K',
  30250: 'Dolby Atmos',
...
const https = (value = '') =>
  value.replace(/^http:/, 'https:').replace(/^\/\//, 'https://');
...
export const isBilibiliUrl = (url: string) =>
  hostnameMatches(url, /(^|\.)bilibili\.com$/i);
export const isBilibiliMediaUrl = (url: string) =>
  hostnameMatches(url, /(^|\.)(bilivideo\.com|akamaized\.net)$/i);
export const videoQualityLabel = (id: number) =>
  QUALITY[id] ?? 'Unknown resolution';
export const audioQualityLabel = (id: number) =>
  AUDIO_QUALITY[id] ?? 'Unknown bitrate';
...
function dash(item: Json): DashStream {
  return {
    ...
    initialization:
      item.SegmentBase?.Initialization ?? item.segment_base?.initialization,
...
function liveSummary(item: Json, nested = false): VideoSummary {
  return {
    ...
    thumbnail: https(
      item.user_cover || item.system_cover || item.cover_from_user,
    ),
    uploader: item.uname ?? '',
    ...
    publishedAt: nested
      ? undefined
      : Date.parse(`${item.live_time}+08:00`) / 1000 || undefined,
    type: 'live',
  };
}
...
function premiumEpisode(item: Json): PremiumEpisode {
  ...
  return {
    ...
    episodeId:
      url.match(/\/(ep\d+)/)?.[1] ?? String(item.ep_id ?? item.id ?? ''),
    episodeNumber: item.title,
    title: entity(
      String(item.share_copy ?? item.long_title ?? item.title ?? '').replace(
        /<[^>]+>/g,
        '',
      ),
    ),
...
export class BilibiliExtractorApi extends BilibiliClient {
  ...
  private async result(url: string, authenticated = false) {
    const envelope = await this.raw<Json>(
      url,
      authenticated
        ? { headers: await bilibiliSession.headers(true) }
        : undefined,
    );
    if (Number(envelope.code) !== 0)
      throw new Error(envelope.message ?? 'Bilibili request failed');
  ...
  async resolveStreamUrl(input: string): Promise<StreamReference> {
    ...
    const premium = parsed.pathname.match(
      /\/bangumi\/play\/(ss\d+|ep\d+)/,
    )?.[1];
    ...
    const rawId =
      parsed.pathname.match(/\/(BV[\w]+|av\d+)/)?.[1] ??
      parsed.searchParams.get('bvid') ??
      (parsed.searchParams.get('aid')
        ? `av${parsed.searchParams.get('aid')}`
        : undefined);
    if (!rawId) throw new Error('Not a Bilibili stream URL');
    const bvid = rawId.startsWith('av')
      ? avToBv(Number(rawId.slice(2)))
      : rawId;
    return {
      kind: 'video',
      id: `${bvid}?p=${page}${timestamp ? `#timestamp=${timestamp}` : ''}`,
      bvid,
      page,
      timestamp,
    };
  ...
  async search(
    ...
    const items: SearchItem[] = (data.result ?? []).map((item: Json) => {
      ...
      if (type === 'bili_user')
        return {
          type: 'channel',
          id: String(item.mid),
          name: String(item.uname).replace(/<[^>]+>/g, ''),
          avatar: https(item.upic),
          description: item.usign,
          followers: Number(item.fans ?? 0),
          videos: Number(item.videos ?? 0),
          verified: false,
        } satisfies ChannelSummary;
      if (type === 'media_bangumi' || type === 'media_ft')
        return {
          bvid: String(item.url ?? item.share_url),
          aid: Number(item.season_id ?? item.media_id ?? 0),
          title: String(item.title ?? '').replace(/<[^>]+>/g, ''),
          thumbnail: https(item.cover),
          uploader: String(item.org_title ?? 'BiliBili').replace(
            /<[^>]+>/g,
            '',
          ),
          duration: Number(item.duration ?? 0) / 1000,
          views: -1,
          publishedAt: Number(item.pubtime ?? item.pub_time ?? 0),
    ...
    return {
      items,
      next: items.length ? String((options.page ?? 1) + 1) : undefined,
    };
  }
  ...
  async getRecommendedLives(page = 1) {
    const data = await this.request<Json>(
      `${LIVE_API}/xlive/web-interface/v1/second/getUserRecommend?page_size=30&platform=web&page=${page}`,
    );
  ...
  async getVideoComplete(bvid: string, page = 1): Promise<VideoDetail> {
    ...
    const [tags, related] = await Promise.all([
      this.getTags(bvid),
      this.getRelated(bvid),
    ]);
    const part = detail.parts[Math.max(0, page - 1)] ?? detail.parts[0];
    return {
      ...detail,
      duration: part?.duration ?? detail.duration,
      tags,
      related,
    };
  }
  ...
  async getTags(bvid: string) {
    const data = await this.request<Json>(
      `${API}/x/web-interface/view/detail/tag?bvid=${encodeURIComponent(bvid)}`,
    );
    return (Array.isArray(data) ? data : []).map((item: Json) =>
  ...
  async getVideoParts(bvid: string): Promise<VideoPart[]> {
    const data = await this.request<Json[]>(
      `${API}/x/player/pagelist?bvid=${encodeURIComponent(bvid)}`,
    );
  ...
  async getPlayback(
    ...
    const progressive: ProgressiveStream[] = (data.durl ?? []).map(
      (item: Json) => ({
        quality,
        label: QUALITY[quality] ?? String(quality),
        url: https(item.url),
        backupUrls: (item.backup_url ?? []).map(https),
        size: item.size,
        mimeType: 'video/mp4',
        headers: {
    ...
    return {
      ...
      flacAudio: data.dash?.flac?.audio
        ? dash(data.dash.flac.audio)
        : undefined,
    };
  }
  ...
  async getSubtitles(
    ...
    const url = await this.wbi('/x/player/wbi/v2', {
      aid: part.aid,
      cid: part.cid,
      isGaiaAvoided: 'false',
      web_location: 1315873,
    ...
    const response = await fetch(url, {
      headers: await bilibiliSession.headers(authenticated),
    });
    const envelope = (await response.json()) as Json;
    if (Number(envelope.code) !== 0)
      throw new Error(envelope.message ?? 'Subtitle request failed');
    const data = envelope.data;
    return (data.subtitle?.subtitles ?? []).map((item: Json) => ({
      id: Number(item.id),
      language: String(item.lan).replace('ai-', ''),
      label: item.lan_doc,
      url: https(item.subtitle_url),
      automatic: Number(item.ai_status) !== 0,
    }));
  }
  ...
  async getVideoShot(bvid: string, cid?: number): Promise<VideoShot> {
    ...
    const durationPerFrame =
      totalFrames > 1 ? ((points.at(-1)! - points[0]) * 1000) / totalFrames : 0;
    return {
      imageUrls: (data.image ?? []).map(https),
      frameWidth: Number(data.img_x_size ?? 0),
      frameHeight: Number(data.img_y_size ?? 0),
      columns: Number(data.img_x_len ?? 10),
      rows: Number(data.img_y_len ?? 10),
      totalFrames,
      durationPerFrame,
      points,
    };
  ...
  async getPremiumSeason(id: string): Promise<PremiumSeason> {
    ...
    const data = await this.result(
      `${API}/pgc/view/web/season?${key}=${encodeURIComponent(id.slice(2))}`,
    );
    ...
    if (!selectedEpisode)
      throw new Error(`Premium episode ${id} was not found`);
    return {
      ...
      uploader: data.up_info
        ? {
            id: String(data.up_info.mid),
            name: String(data.up_info.uname),
            avatar: https(data.up_info.avatar),
          }
        : undefined,
  ...
  async getChannel(mid: string): Promise<Channel> {
    const [profile, live] = await Promise.all([
      this.request<Json>(
        `${API}/x/web-interface/card?photo=true&mid=${encodeURIComponent(mid)}`,
      ),
      this.request<Json>(
        `${LIVE_API}/room/v1/Room/get_status_info_by_uids?uids[]=${encodeURIComponent(mid)}`,
      ),
    ]);
    ...
    return {
      id: mid,
      name: card.name,
      avatar: https(card.face),
      banner: https(profile.space?.l_img),
      description: card.sign ?? '',
      followers: Number(card.fans ?? 0),
      live: liveData?.live_status ? liveSummary(liveData, true) : undefined,
    };
  }
  ...
  async getChannelVideos(
    ...
    if (options.mode === 'client') {
      ...
      const data = await this.request<Json>(
        `https://app.bilibili.com/x/v2/space/archive/cursor?${new URLSearchParams(Object.entries(params).map(([key, value]) => [key, String(value)])).toString()}`,
      );
      const items = (data.item ?? []).map((item: Json) =>
        normalizeSummary({ ...item, pic: item.cover }),
      );
      return {
        items,
        next: items.length ? String(bvToAv(items.at(-1)!.bvid)) : undefined,
      };
    }
    const path =
      options.mode === 'search'
  ...
  async getChannelPlaylists(
    mid: string,
    page = 1,
  ): Promise<PageResult<RemotePlaylist>> {
    const data = await this.request<Json>(
      `${API}/x/polymer/web-space/seasons_series_list?mid=${encodeURIComponent(mid)}&page_num=${page}&page_size=10`,
    );
    const seasons = (data.items_lists?.seasons_list ?? []).map(
      (item: Json) => ({
        id: String(item.meta.season_id),
        type: 'season' as const,
        name: item.meta.name,
        thumbnail: https(item.meta.cover),
        streamCount: Number(item.meta.total),
      }),
    );
    const series = (data.items_lists?.series_list ?? []).map((item: Json) => ({
    ...
    const items = [...seasons, ...series].filter(
      (item) => item.streamCount > 0,
    );
    return { items, next: items.length ? String(page + 1) : undefined };
  }
  ...
  async getRecordedDanmaku(cid: number): Promise<Danmaku[]> {
    const response = await fetch(`${API}/x/v1/dm/list.so?oid=${cid}`, {
      headers: await bilibiliSession.headers(),
    });
    const compressed = new Uint8Array(await response.arrayBuffer());
    let xml: string;
    try {
      xml = new TextDecoder().decode(inflateRaw(compressed));
    } catch {
      xml = new TextDecoder().decode(compressed);
    }
    if (/<state>1<\/state>/.test(xml)) return [];
    return [...xml.matchAll(/<d p="([^"]+)">([\s\S]*?)<\/d>/g)].flatMap(
      (match) => {
        ...
        try {
          ...
          if (Array.isArray(array) && typeof array[4] === 'string' && array[4])
            text = array[4];
        } catch {
          /* Standard danmaku content is plain text. */
        }
        return [
          {
            text,
            color: 0xff000000 + Number(attr[3]),
            position:
              mode === '4'
                ? ('bottom' as const)
                : mode === '5'
                  ? ('top' as const)
                  : ('regular' as const),
            fontScale: attr[2] === '18' ? 0.5 : attr[2] === '36' ? 0.7 : 0.6,
            time: Number(attr[0]) + 2.5,
            live: false,
          },
        ];
      },
  ...
  async getLiveRoom(id: string): Promise<LiveRoom> {
    const init = await this.request<Json>(
      `${LIVE_API}/room/v1/Room/room_init?id=${encodeURIComponent(id)}`,
    );
    const statuses = await this.request<Json>(
      `${LIVE_API}/room/v1/Room/get_status_info_by_uids?uids[]=${init.uid}`,
    );
    ...
    return {
      ...
      tags: [room.tag_name, ...String(room.tags ?? '').split(',')].filter(
        Boolean,
      ),
    };
  }
  ...
  async getLiveStream(roomId: number) {
    const data = await this.request<Json>(
      `${LIVE_API}/room/v1/Room/playUrl?qn=10000&platform=web&cid=${roomId}`,
    );
  ...
  async getRoundPlay(
    ...
    const round = await this.result(
      `https://api.live.bilibili.com/live/getRoundPlayVideo?room_id=${roomId}&a=${timestamp}&type=flv`,
    );
    if (Number(round.cid) < 0) throw new Error('Round play is unavailable');
    const playback = await this.request<Json>(
      `${API}/x/player/playurl?cid=${round.cid}&bvid=${round.bvid}&fnval=4048&qn=120&fourk=1&try_look=1`,
    );
    return {
      bvid: String(round.bvid),
      cid: Number(round.cid),
      title: String(round.title),
      playTime: Number(round.play_time),
      nextTimestamp: timestamp + Number(playback.dash?.duration ?? 0) * 1000,
      video: (playback.dash?.video ?? []).map(dash),
      audio: (playback.dash?.audio ?? []).map(dash),
    };
  }
  ...
  async getChannelVideosWithFallback(mid: string, page = 1) {
    ...
    for (const mode of ['web', 'search', 'client'] as const) {
      try {
        return await this.getChannelVideos(mid, { page, mode });
      } catch (error) {
        failures.push(error);
      }
    }
    throw new AggregateError(
      failures,
      'All Bilibili creator-video APIs failed',
    );
  }

  async getLiveDanmakuCredentials(roomId: number) {
    const data = await this.request<Json>(
      `${LIVE_API}/xlive/web-room/v1/index/getDanmuInfo?type=0&id=${roomId}`,
    );
