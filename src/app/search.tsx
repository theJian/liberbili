...
export default function SearchScreen() {
  ...
  const search = useCallback(async (nextPage = 1) => {
    ...
    } catch (value) { setError(value as Error); } finally { setLoading(false); }
  }, [loading, query]);
...
const styles = StyleSheet.create({
  container: { flex: 1 }, searchBar: { margin: 12, borderRadius: 14, flexDirection: 'row', paddingLeft: 14, overflow: 'hidden' }, input: { flex: 1, height: 48, fontSize: 16 }, submit: { width: 54, alignItems: 'center', justifyContent: 'center' }, submitText: { color: '#fff', fontSize: 24, textAlign: 'center' }, footer: { padding: 20 },
});
